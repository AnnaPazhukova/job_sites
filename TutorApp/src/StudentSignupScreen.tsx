...
import { ensureLinked } from "./lib/studentAuth";
...
export function StudentSignupScreen({ code, onLinked }: Props) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      if (mode === "signup") {
        ...
        if (data.session) {
          await ensureLinked(code);
          onLinked();
        } else {
          setInfo("Проверьте почту и подтвердите регистрацию по ссылке в письме, затем вернитесь по этой же ссылке-приглашению и войдите.");
        }
      } else {
    ...
    <div style={{ fontFamily: "'Inter', -apple-system, BlinkMacSystemFont, sans-serif" }} className="min-h-screen bg-[#F7F8FA] flex items-center justify-center p-4">
      <Card className="w-full max-w-sm p-6">
        ...
        <p className="text-sm text-gray-500 text-center mb-6">
          {mode === "signup" ? "Регистрация личного кабинета ученика" : "Вход в личный кабинет ученика"}
        </p>
        ...
        >
          {mode === "signin" ? "Ещё нет аккаунта? Зарегистрироваться" : "Уже зарегистрированы? Войти"}
        </button>
      </Card>
    </div>
  );
